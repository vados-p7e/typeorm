    ...
    QueryBuilder.prototype.createWhereIdsExpression = function (ids) {
        ...
        var whereStrings = ids.map(function (id, index) {
            ...
            metadata.parentIdColumns.forEach(function (parentIdColumn, secondIndex) {
                ...
                var parameterName = "parentId_" + index + "_" + secondIndex;
                whereSubStrings.push(alias + _this.escape(parentIdColumn.databaseName) + " = " + _this.connection.driver.createParameter(parameterName, parameterIndex));
                _this.expressionMap.nativeParameters[parameterName] = parentIdColumn.getEntityValue(id);
                parameterIndex++;
            });
        ...
        if (where instanceof Brackets_1.Brackets) {
            ...
            if (this.expressionMap.mainAlias.hasMetadata) {
                andConditions = wheres.map(function (where, whereIndex) {
                    var propertyPaths = EntityMetadataUtils_1.EntityMetadataUtils.createPropertyPath(_this.expressionMap.mainAlias.metadata, where);
